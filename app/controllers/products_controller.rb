        ...
        if params[:submit_button]
          ...
          if ["yes", "no"].include? submit
            if submit == "no"
              ...
              flash[:notice] = (@last_offer.price == 0) ? "Cool! Congrats! you won free Dealkat khakis. Share with facebook /twitter friends!" : "Cool! You snagged a deal!"
            end
          ...
          if @last_offer and @last_offer.counter?
              ...
              if @offer
                if price >= @last_offer.price
                  @last_offer.update_attribute(:response, "accepted")
                  flash[:notice] = "Cool! You snagged a deal!"
                  return
                end
                if(price > @offer.price)
                    ...
                    @new_offer = with_precision((@product.reg_price.to_f*(rand_per.to_f/100)), :precision => 0).to_f.ceil

                    #render :text => [@new_offer, @offer.price, @last_offer.price].inspect and return false
                    ...
                    elsif @new_offer <= with_precision(@product.target_price, :precision => 0).to_f.ceil
              ...
              if(price <= @product.min_price)
                  @new_offer = with_precision((@product.reg_price.to_f*0.95), :precision => 0).to_f.ceil
                  ...
                  @new_offer = with_precision((@product.reg_price.to_f*(rand_per.to_f/100)), :precision => 0).to_f.ceil
                  if @new_offer <= with_precision(@product.target_price, :precision => 0).to_f.ceil
                    @new_offer = @product.target_price
                    Offer.create(:ip => request.remote_ip, :token => offer_token, :product_id => @product.id, :price => @new_offer, :response => "last", :counter => 1)
